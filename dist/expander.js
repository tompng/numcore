...
const { EQNAN } = util_1.RangeResults;
exports.GAPMARK = '/*GAP*/';
exports.NANMARK = '/*NAN*/';
...
const add = ([a, b], namer) => {
    ...
    const minvar = namer();
    const maxvar = namer();
    const mincode = `const ${minvar}=${typeof a === 'number' ? a : a[0]}+${typeof b === 'number' ? numToSafeString(b) : b[0]}`;
    const maxcode = `const ${maxvar}=${typeof a === 'number' ? a : a[1]}+${typeof b === 'number' ? numToSafeString(b) : b[1]}`;
...
const sub = ([a, b], namer) => {
    ...
    const minvar = namer();
    const maxvar = namer();
    const mincode = `const ${minvar}=${typeof a === 'number' ? a : a[0]}-${typeof b === 'number' ? numToSafeString(b) : b[1]}`;
    const maxcode = `const ${maxvar}=${typeof a === 'number' ? a : a[1]}-${typeof b === 'number' ? numToSafeString(b) : b[0]}`;
    return [[minvar, maxvar], `${mincode};${maxcode}`];
};
const minus = ([a], namer) => {
    if (typeof a === 'number')
...
function powVarPositive([vmin, vmax], b, namer) {
    if (Math.floor(b) === b)
...
const mult = ([a, b], namer) => {
    ...
    if (typeof a === 'number' || typeof b === 'number') {
        ...
        const [minname, maxname] = typeof a !== 'number' ? a : b;
        if (n === 0)
            return [0, ''];
...
const atan2 = (args, namer) => {
    ...
    if (typeof x === 'number') {
        ...
        const code = [
            ...
            `let ${minvar},${maxvar}`,
            x > 0 ? assign : `if(${ymin}<0&&${ymax}>0){${exports.GAPMARK};${minvar}=${-Math.PI};${maxvar}=${Math.PI}}else{${assign}}`
        ].join(';');
        return [[minvar, maxvar], code];
    }
    const [xmin, xmax] = x;
...
const atanOverload = (args, namer) => {
    if (args.length === 2)
        return atan2(args, namer);
    return atan(args, namer);
};
const fact = (args, namer) => {
