// parameters calculated by https://gist.github.com/tompng/a5752c86a5cdccffa17d6c1b5de24b9a
...
export const factorial = eval(`${factorialCode}; factorial`)
...
export function expandFactorial(
  ...
  namer: NameGenerator,
  GAPMARK: string
): [number | MinMaxVarName, string] {
  if (typeof x === 'number') return [factorial(x), '']
  const [xmin, xmax] = x
  const minvar = namer()
  const maxvar = namer()
    ...
    if(-1 < ${xmin}){
      if (${xmin} < ${positiveStationaryPoint}) {
        if (${positiveStationaryPoint} < ${xmax}) {
          ...
          ${minvar} = ${factorial(positiveStationaryPoint)};
        } else {
